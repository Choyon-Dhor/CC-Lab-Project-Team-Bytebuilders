...
static int is_assignable(const char *target_type, const char *expr_type) {
    if (!target_type || !expr_type) return 0;
    if (strcmp(target_type, expr_type) == 0) return 1;
    if (strcmp(target_type, "float") == 0 && strcmp(expr_type, "int") == 0) return 1;
    return 0;
}
...
static const char *infer_expr_type(ASTNode *node, SymbolTable *table, int *error_count) {
    ...
    switch (node->type) {
        ...
        case NODE_UNOP: {
            ...
            if (strcmp(node->op, "-") == 0) {
                if (is_numeric(operand_type)) return operand_type;
                semantic_error(node->line, "invalid expression: unary '-' requires a numeric operand");
                (*error_count)++;
                return "unknown";
            }
            if (strcmp(node->op, "!") == 0) {
                if (strcmp(operand_type, "bool") == 0) return "bool";
